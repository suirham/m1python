...
def test_points():
  assert ranking.points(2, 7) == 13
  assert ranking.points(7, 1) == 22
  assert ranking.points(-1, 297) == 294
...
def test_goal_for_count_during_a_match():
  ...
  assert ranking.goal_for_count_during_a_match(4, match1) == 2 and ranking.goal_for_count_during_a_match(2, match1) == 2
  assert ranking.goal_for_count_during_a_match(3, match2) == 1
  assert ranking.goal_for_count_during_a_match(2, match2) == 3

def test_goal_against_count_during_a_match():
  assert ranking.goal_against_count_during_a_match(1, match0) == 5
  assert ranking.goal_against_count_during_a_match(3, match0) == 3
  assert ranking.goal_against_count_during_a_match(4, match1) == 2 and ranking.goal_for_count_during_a_match(2, match1) == 2
